...
describe Optimizely::ConditionTreeEvaluator do
  ...
  describe 'and evaluation' do
    it 'should return true when ALL conditions evaluate to true' do
      leaf_callback = ->(_condition) { return true }
      expect(@condition_evaluator.evaluate(['and', @browser_condition, @device_condition], leaf_callback)).to be true
    end
    ...
    it 'should return false if one condition evaluates to false' do
      ...
      expect(@condition_evaluator.evaluate(['and', @browser_condition, @device_condition], leaf_callback)).to be false
    end
    ...
    describe 'nil handling' do
      ...
      it 'should return false when operands evaluate to trues, falses, and nils' do
        ...
        expect(@condition_evaluator.evaluate(['and', @browser_condition, @device_condition, @location_condition], leaf_callback)).to be false
      end
    end
  end
  ...
  describe 'or evaluation' do
    it 'should return false if all conditions evaluate to false' do
      leaf_callback = ->(_condition) { return false }
      expect(@condition_evaluator.evaluate(['or', @browser_condition, @device_condition], leaf_callback)).to be false
    end

    it 'should return true if any condition evaluates to true' do
      leaf_callback = double
      allow(leaf_callback).to receive(:call).and_return(false, true)
      expect(@condition_evaluator.evaluate(['or', @browser_condition, @device_condition], leaf_callback)).to be true
    end

    describe 'nil handling' do
      it 'should return nil when all operands evaluate to nil' do
        leaf_callback = ->(_condition) { return nil }
        expect(@condition_evaluator.evaluate(['or', @browser_condition, @device_condition], leaf_callback)).to eq(nil)
      end
      ...
      it 'should return true when operands evaluate to trues, falses, and nils' do
        leaf_callback = double
        allow(leaf_callback).to receive(:call).and_return(true, false, nil)
        expect(@condition_evaluator.evaluate(['or', @browser_condition, @device_condition, @location_condition], leaf_callback)).to be true
      end
    end
  end
  ...
  describe 'not evaluation' do
    it 'should return true if the condition evaluates to false' do
      leaf_callback = ->(_condition) { return false }
      expect(@condition_evaluator.evaluate(['not', @browser_condition], leaf_callback)).to be true
    end

    it 'should return false if the condition evaluates to true' do
      leaf_callback = ->(_condition) { return true }
      expect(@condition_evaluator.evaluate(['not', @browser_condition], leaf_callback)).to be false
    end
    ...
    it 'should return the result of negating the first condition, and ignore any additional conditions' do
      ...
      expect(@condition_evaluator.evaluate(%w[not 1 2 1], leaf_callback)).to be false

      leaf_callback2 = ->(id) { return id == '2' }
      expect(@condition_evaluator.evaluate(%w[not 1 2 1], leaf_callback2)).to be true

      leaf_callback3 = ->(id) { return id == '1' ? nil : id == '3' }
      expect(@condition_evaluator.evaluate(%w[not 1 2 3], leaf_callback3)).to eq(nil)
    end

    describe 'nil handling' do
      it 'should return nil when operand evaluates to nil' do
        leaf_callback = ->(_condition) { return nil }
        expect(@condition_evaluator.evaluate(['not', @browser_condition, @device_condition], leaf_callback)).to eq(nil)
      end

      it 'should return nil when there are no operands' do
        leaf_callback = ->(_condition) { return nil }
        expect(@condition_evaluator.evaluate(['not'], leaf_callback)).to eq(nil)
      end
    end
  end
  ...
  describe 'implicit operator' do
    it 'should behave like an "or" operator when the first item in the array is not a recognized operator' do
      leaf_callback = double
      allow(leaf_callback).to receive(:call).and_return(true, false)
      expect(@condition_evaluator.evaluate([@browser_condition, @device_condition], leaf_callback)).to be true

      leaf_callback = ->(_condition) { return false }
      expect(@condition_evaluator.evaluate([@browser_condition, @device_condition], leaf_callback)).to be false
    end
  end
end
