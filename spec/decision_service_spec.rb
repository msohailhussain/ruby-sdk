...
describe Optimizely::DecisionService do
  ...
  describe '#get_variation' do
    ...
    it 'should return the correct variation ID (using Bucketing ID attrbiute)
        for a given user for whom a variation has been forced' do
      user_attributes = {
      ...
      expect(
        decision_service.get_variation(
    ...
    it 'should return correct variation ID (using Bucketing ID attrbiute)
        if user ID is in whitelisted Variations and variation is valid' do
      user_attributes = {
        'browser_type' => 'firefox',
        OptimizelySpec::RESERVED_ATTRIBUTE_KEY_BUCKETING_ID => 'pid'
      }
    ...
    it 'should return the correct variation ID for a user in a whitelisted variation
          (even when audience conditions do not match)' do
      user_attributes = {'browser_type' => 'wrong_browser'}
      expect(
        decision_service.get_variation(
          'test_experiment_with_audience',
          'forced_audience_user',
          user_attributes
        )
      ).to eq('122229')
      expect(spy_logger).to have_received(:log).once.with(
    ...
    it 'should bucket normally if user is whitelisted into a forced variation that is not in the datafile' do
      ...
      expect(decision_service.bucketer).to have_received(:bucket).once.with(
        experiment,
       'forced_user_with_invalid_variation', 'forced_user_with_invalid_variation'
      )
    end
  ...
  describe '#get_variation_for_feature_experiment' do
    ...
    describe 'when the feature flag\'s experiment ids array is empty' do
      it 'should return nil and log a message' do
        ...
        expect(
          decision_service.get_variation_for_feature_experiment(feature_flag, 'user_1', user_attributes)
        ).to eq(nil)
      ...
      describe 'and the user is not bucketed into the feature flag\'s experiments' do
        ...
        it 'should return nil and log a message' do
          ...
          expect(spy_logger).to have_received(:log).once
            .with(
              Logger::INFO,
              "The user 'user_1' is not bucketed into any of the experiments on the feature 'multi_variate_feature'."
            )
        end
      end
    ...
    describe 'when the feature flag is associated with a mutex experiment' do
      ...
      describe 'and the user is bucketed into one of the experiments' do
        ...
        it 'should return the variation the user is bucketed into' do
          ...
          expect(
            decision_service.get_variation_for_feature_experiment(feature_flag, user_id, user_attributes)
          ).to eq(expected_decision)
    ...
    describe 'when the user qualifies for targeting rule' do
      describe 'and the user is bucketed into the targeting rule' do
        it 'should return the variation the user is bucketed into' do
          ...
          expect(
            decision_service.get_variation_for_feature_rollout(feature_flag, user_id, user_attributes)
          ).to eq(expected_decision)
          expect(spy_logger).to have_received(:log).once
  ...
  describe '#get_variation_for_feature' do
    ...
    describe 'when the user is bucketed into the feature experiment' do
      it 'should return the bucketed experiment and variation' do
        ...
        expect(
          decision_service.get_variation_for_feature(feature_flag, user_id, user_attributes)
        ).to eq(expected_decision)
      end
    end
